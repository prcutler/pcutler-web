jQuery(function(){
	...
	jQuery(".tw-link img").mouseover(function(){
		jQuery(this).csChop({pieceDelay:20,ease:"ease-in-out",hPieces:29,vPieces:29,type:"horizontal",rotate:360,rotateSymmetric:true,origin:"center center",onStart:function(c){
				c.slices().each(function(){
					jQuery(this).html('<a href="'+c.src().parent().attr("href")+'" target="_blank" style="display:block; width:100%;height:100%"></a>')	
				})
			}},2600,function(a){
				a.reset()
		})	
	})
	jQuery(".fb-link img").mouseover(function(){
		jQuery(this).csChop({pieceDelay:20,ease:"ease-in-out",hPieces:29,vPieces:29,type:"vertical",rotate:-360,rotateSymmetric:true,origin:"center center",onStart:function(c){
				c.slices().each(function(){
					jQuery(this).html('<a href="'+c.src().parent().attr("href")+'" target="_blank" style="display:block; width:100%;height:100%"></a>')	
				})
			}},2500,function(a){
	...
	jQuery(".demo-icon-1").click(function(){
		jQuery(this).csChop({
			type:"horizontal",
			hPieces:35,
			rotate:360,
	...
	jQuery(".demo-icon-2").mouseover(function(){
		jQuery(this).csChop({
			type:"horizontal",
			hPieces:35,
			rotate:360,
	...
	jQuery(".demo-icon-3").click(function(){
		jQuery(this).csChop({
			type:"vertical",
			xOffset:20,
			vPieces:35,
	...
	jQuery(".demo-icon-4").mouseover(function(){
			jQuery(this).csChop({
				origin:"10px 50%",
				type:"multi",
				hPieces:7,
			...
			function(x3){
				x3.setTo({origin:"5px 50%",rotate:0},1000,function(){
					...
					jQuery(this).css("cursor","pointer").hover(function(){jQuery(this).fadeTo(300,0.5)},
						function(){jQuery(this).fadeTo(300,1)});
	...
	jQuery(".gallery-thumbs").click(function(){
		jQuery(".gallery-text").fadeOut(600)
		jQuery(this).csChop(
			{
				type:"multi",
				hPieces:2,
	...
	function initGalleryThumbs(cs) {
		...
		.each(function(){
			jQuery(this).hover(
				function(){
					jQuery(this).csTransform({transform:"rotate(355deg) scale(1.2)",time:500})
				},
				function(){
					jQuery(this).csTransform({transform:"rotate(360deg)  scale(1)",time:500})
				}
			)
			
			jQuery(this).click(function(){
				var sliceIndex = jQuery(this).index();
				var rowIndex = jQuery(this).parent().index();
	...
	function showGalSlide(slideIndex) {
		...
		jQuery(".gal-slide-"+slideIndex).csChop(
			{
				hidden:true,
			...
			function(cs) {
				...
				cs.setTo({scaleX:1,scaleY:1,rotate:0,xOffset:0,yOffset:0,pieceDelay:130},2000,function(){
					...
					jQuery(".gallery-slides img:not(.gal-slide-"+slideIndex+")").each(function(){
						var csid = jQuery(this).attr("data-csid");
						if(csid&&csid!="") {
							jQuery(this).attr({"data-csid":""});
							jQuery(".csChopper[data-csid='"+csid+"']").remove()	
						}
					})	
				})	
			}
	...
	jQuery(".g-green").click(function(){
		jQuery(".p-green").csChop(
			{
				type:"multi",
				allowMultiChop : true,
				hPieces:10,
	...
	jQuery(".g-blue").click(function(){
		jQuery(".p-blue").csChop(
			{
				type:"multi",
				allowMultiChop : true,
				hPieces:10,
	...
	jQuery(".icons a").click(function(e){
		e.preventDefault();
	...
	jQuery(".t-custom").click(function(){
		if(jQuery("#custom-transition").css("display")=="none") {
			jQuery("#custom-transition").slideDown(600)	
		}
		else {
			jQuery("#custom-transition").slideUp(600)
		}
	})
	jQuery("#custom-form").submit(function(e){
		e.preventDefault();
		var a = {};
		a.autoplay = false;	
		a.type = jQuery("#type option:selected").val();
		a.xOffset = jQuery("#xOffset").val()*1;
		a.yOffset = jQuery("#yOffset").val()*1;
		a.hPieces = jQuery("#hPieces").val()*1;
		a.vPieces = jQuery("#vPieces").val()*1;
		a.rotate = jQuery("#rotate").val()*1;
		a.rotateSymmetric = jQuery("#symm option:selected").val()=="true"?true:false;
		a.scaleX = jQuery("#scaleX").val()*1;
		a.scaleY = jQuery("#scaleY").val()*1;
		a.translateX = jQuery("#translateX").val()*1;
		a.translateY = jQuery("#translateY").val()*1;
		a.dur1 = jQuery("#dur1").val()*1;
		a.dur2 = jQuery("#dur2").val()*1;
		a.dur3 = jQuery("#dur3").val()*1;
		a.pieceDelay = jQuery("#pieceDelay").val()*1;
		a.xFadeDelay = jQuery("#xFadeDelay").val()*1;
		var originLeft = jQuery("#originLeft").val();
		var originTop = jQuery("#originTop").val();
	...
	jQuery("#kickassable").click(function(e){
		e.preventDefault();
		...
		var kac = {};
		jQuery(".dark-layer").fadeIn(600,function(){
			jQuery(".slider,#slide-loader,#transitions,#new-methods,#gallery,#generator,#akbt,#content,.csChopper").remove()
			jQuery(".pr-i").delay(8000).fadeIn(8000,function(){
					jQuery(this).csChop({
						type:"horizontal",
						translateX:-1000,
						hPieces:20,
				...
				function showWhiteSlides() {
					jQuery(".white-layer").fadeIn(3500,function(){
						jQuery(".csChopper").remove()
						jQuery(".pr-5").csChop({
							type:"multi",
							xOffset:-1000,
							yOffset:600,
																			...
																			pr5.fadeToSet({xFade:true,image:".pr-8",scaleY:1,sclaleX:1,rotate:0,xOffset:0, yOffset:0,fadeDur:300},9000,function(){
																				pr5.setTo({origin:"50% 50%",scaleY:-1,sclaleX:-1,xOffset:-1000, yOffset:-600, rotate:360,pieceDelay:250,completeFirst:function(){
																						pr5.fadeToSet({xFade:true,image:".pr-9",scaleY:1,sclaleX:1,rotate:0,xOffset:0, yOffset:0,fadeDur:300},9000,function(){
																								...
																								setTimeout(function(){
																									jQuery(".white-layer").csChop({zIndex:38000,onStart:function(cs2){cs2.slices().css("background-color","#fff")},type:"multi",hPieces:5,vPieces:5,scaleX:0, scaleY:0, xOffset:-500, translateX:-100, yOffset:-200,rotate:720,rotateSymmetric:false,pieceDelay:400},7000,function(){jQuery(".white-layer,.present-slides,.csChopper").remove()})																									
																								},5000)
																						})
																					}
																				},9000)	
