...
class EasyContactFormsFiles extends EasyContactFormsBase {
	...
	function getFileDownloadLink($doctype, $docfield, $docid, $open = FALSE) {
		...
		$ds = DIRECTORY_SEPARATOR;
		if ($filedata->Webdir) {
			return EASYCONTACTFORMS__engineWebAppDirectory . '/' .
	...
	function protectedUpload($_uldmap, $filespecmap = NULL) {
		...
		$filesize = intval($filespecmap['size']);
		$filetype = EasyContactFormsUtils::addMSlashes($filespecmap['type']);
		...
		$query
			= "DELETE FROM #wp__easycontactforms_files WHERE Doctype='$Type' AND Docid='$id' AND Docfield='$fieldname'";
		EasyContactFormsDB::query($query);
		...
		if ($Type == "Files") {
			$filespec->id = $isid;
			$Storagename = EasyContactFormsFiles::getStorageFileName($filespec);
			$valuemap['Docid'] = $isid;
